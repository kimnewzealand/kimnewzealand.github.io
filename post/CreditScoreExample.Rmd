...
Credit Risk modeling predicts whether a customer or applicant will default on his or her loan.  The models include predictor variables that are categorical or numeric. Since one of the key outputs in the modeling process is a credit scorecard with discrete attributes to allocate scores, the variables need to be discretised. We will refer to this grouping process as binning.
...
Here we will use a public dataset, German Credit Data, with a binary response variable, good or bad risk. We will evaluate and compare the models with typical credit risk model measures, AUC, Kolmogorov-Smirnov(KS) and Gini.
...
The German Credit data set is available from the [UCI Machine Learning Repository Set](https://archive.ics.uci.edu/ml/support/statlog+(german+credit+data)). We will load the data  with [httr](https://cran.r-project.org/web/packages/httr/index.html) and [readr](https://cran.r-project.org/web/packages/readr/index.html) R packages. 

Note Readr is available from [loading and attaching the package to the search path](http://r-pkgs.had.co.nz/namespace.html#search-path) with library(tidyverse) above.

```{r load data}
# Set the url and uset GET function from httr to import the csv file to a temporary local directory, and read_csv from readr R package. This csv is also available at this url.
...
This is a relatively small dataset with  `r dim(creditdata)[1]` observations and `r dim(creditdata)[2]` variables. 
...
creditdata <- creditdata %>% 
      ...
      data.frame() # leave the factors in except for the default value
...
As per the [metadata](https://archive.ics.uci.edu/ml/datasets/statlog+(german+credit+data)), there are no missing values. Let's confirm with a missing values plot, using the plot_missing function from Data Explorer package.

[DataExplorer](https://cran.r-project.org/web/packages/DataExplorer/index.html) speeds up data exploration process and automatically scans through each variable and does data profiling.

```{r missing plot}
plot_missing(creditdata)
```
```{r base copy}
# Recode the default value to 0 and 1 for the modeling with binary outcomes
...
The response variable is `default` classifies people described by a set of attributes as good or bad credit risks, as per the the abstract. This type of data could be used to make a loan application decision. As per the metadata  `default` values are 1 = Good or 2 = Bad, however the variable has been coded to 0 = Good and 1 = Bad in the dataset. 
...
We will include the row proportions as indicators of the proportion good and proportion bad for each value.
...
From the table, 30% of the loan applications are risky in this dataset. If this was representative of the population then 30% of applicants cannot repay their loans. Given the Chi-squared test, we can reject a null hypothesis that these two variables, `default` and `checkingstatus1`, are independent.

The imbalance in the dataset is worth noting, however real life datasets are typically more highly imbalanced.
...
Let's calculate the information values for the dataset, sorted from highest importance or predictive strength to the lowest. Then plot the variables with highest importance and the corresponding WOE values. A positive WOE for a bin value indicates the proportion of good is higher than the proportion of bad, ie the log odds of the row proportions from the CrossTable above.

```{r WOE, message=FALSE, warning=FALSE}
...
IV$Summary %>% 
      knitr::kable()
# Plot the top 6 variables WOE bar charts. Raised potential issue with woe signage in GitHub https://github.com/klarsen1/Information/issues/2
...
The following variables appear to be linked to riskier applicants:  
...
Let's now view the `checkingstatus1` with `purpose` and `duration` as a boxplot by `default` to view the relationships between these variables.
...
Split the data into training and test sets following the Data Camp course steps in [Chapter 1](https://campus.datacamp.com/courses/introduction-to-credit-risk-modeling-in-r/chapter-1-introduction-and-data-preprocessing?ex=12). Here we will use a 70:30 split between training and test sets.

We will create two sets of splits, one training and test set for the feature engineered coarse binning and training_base and test_base for the woe binning.
...
Fit the first logistic model on the training data set where the coarse binned variables.
...
Make predictions on the test set and evaluate the model using AUC, [KS](https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test) and Gini measures with the [pROC](https://cran.r-project.org/web/packages/pROC/index.html), [ROCR](https://cran.r-project.org/web/packages/ROCR/index.html) and [scoredcard](https://cran.r-project.org/web/packages/scorecard/index.html) R packages.
...
tree <- rpart(formula = default~., data = training, method="class", cp=0.001,parms = list(split="information"))
# Plot the cp complexity parameter of the tree
plotcp(tree)
# Prune the tree at the minimum cp value
ptree <- prune(tree, cp= tree$cptable[which.min(tree$cptable[,"xerror"]),"CP"])
# Plot the tree
rpart.plot(ptree,type=1, fallen.leaves = FALSE, extra=4)
```
...
prediction_tree_probs <- predict(ptree, newdata = test,type="prob")[,2]
# Calculate the ROC
...
We will fit another logistic model on the training set. This model will create WOE bins using the [scorecard](https://cran.r-project.org/web/packages/scorecard/index.html) R package.

```{r model 3}
# Weight of average (WOE) binning generates optimal binning for numerical, factor and categorical variables>. Note the default values are still 1 and 2.
bins <- woebin(training_base, y="default",max_num_bin = 5)
...
The WOE bin has grouped the `age` similarly to the coarse bins up until 35 where it has added a group for 35:37. The `duration` WOE bin has also grouped by 12 months initially and not annually thereafter. For `amount` there are 5 WOE bins as opposed to 3 bins. The WOE binning has also grouped catageorical variables; WOE binning has grouped `checkingstatus1` less than 0 and 0 to 200DM levels.
...
```{r tree model woe}
# Let's fit a classification model on the training set containing woe values
tree_woe <- rpart(formula = default~., 
                  data = training_woe, 
                  method="class", 
                  cp=0.001, 
...
# Plot CP
plotcp(tree_woe)
# Prune the tree based on min CP
ptree_woe <- prune(tree_woe, cp= tree$cptable[which.min(tree$cptable[,"xerror"]),"CP"])
# Plot the pruned tree 
rpart.plot(ptree_woe,type=1, fallen.leaves = FALSE, extra=4)
```
  
In the last two models, we converted the training and test sets original input data to WOE values. Therefore this tree splits based on woe values, not the bin values as per the first tree fitted on coarse binning of numerical variables. 

```{r predict rpart woe}
# Calculate the prediction probabilities on the test data, using type prob and selecting the second column which is the probabilty of the default =1
prediction_tree_woe_probs <- predict(ptree_woe, newdata = test_woe,type="prob")[,2]
# Calculate the ROC
...
Now create a scorecard on the Model 1, which has the highest AUC, using the scorecard package.
...
The baseline model in this case could be the `default` split of 0.7 as AUC.

We compare the AUC of each of the four models, logistic and tree models with different manual and optimised binning methods as follows:

```{r compare results}
results <- cbind(c("Model 1 - Logistic Model with Coarse Binning","Model 2 - Tree Model with Coarse Binning","Model 3 - Logistic Model with WOE Binning","Model 4 - Tree Model with WOE Binning"),c(AUC,AUC_tree,AUC3,AUC_tree_woe)) %>% 
      data.frame()
names(results) <- c("Model","AUC")
results %>%  
      knitr::kable(digits=c(0,2))
```

It appears that all models are better than the baseline model.

The tree models perform worse than the logistic models and the WOE binning  did not improve the models from the manual binning. The best model overall appears to be the logistic model with coarse binning.  This is surprising, as I was expecting an optimised woe process to create a better model.

In this exercise we have included various plots to visualise calculations. The woebin_plots are useful for visualising the bins created and associated probabilities. The tree plots are useful for interpretability of the tree models.

Since this has been an exercise in comparing binning methods, in a separate exercise these models could be improved with further feature selection, engineering or parameter tuning.
