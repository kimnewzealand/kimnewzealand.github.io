...
# Plot the male population density
nz_region_f$tooltipm <- paste0(nz_region_f$REGC2017_N, "  : ", round(nz_region_f$maledens2013), " per sq km")

g <- ggplot(nz_region_f, aes(long, lat,group = group)) +
      geom_polygon_interactive(aes(fill=maledens2013, 
                               tooltip = htmltools::htmlEscape(tooltipm, TRUE))) +
      geom_path(color = "white") + 
      coord_map() +
     scale_fill_distiller(type="seq", trans="reverse", palette = "BuPu",   name="Population density") +
      theme_void() +   
      labs(color="Pop per sq km)") +
      ggtitle("New Zealand Male Population Density in 2013")

# Now plot the g object using ggiraph
ma2013 <- ggiraph(code = print(g))
ma2013
