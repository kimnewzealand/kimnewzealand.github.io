...
image : "img/portfolio/forcediag.png"
# showonlyimage : false
weight : 5
---
...
inst_intern_count <- combine %>%
      distinct(Student.institution,internship,token) %>%
      group_by(Student.institution,internship) %>%  
      summarize(counts = n()) %>%
      ungroup() %>%  
      arrange(desc(counts))
# Replace the Event NA's to Other
inst_intern_count$Event.institution <- replace_na(inst_intern_count$Student.institution,"Other")
# Since there is a large number of institutions, Group the institutions with count less than 10 as Other
l <- length(inst_intern_count$counts)
inst_intern_count$Student.institution[15:l] <- "Other"
# Check that the total intern tokens in original file is same as sum in this summary
length(unique(intern$token))==
inst_intern_count %>% filter(internship=="Successful") %>% select(counts) %>% sum()

# Create the links and nodes for the Sankey and force network diagrams
name_vec <- c(unique(inst_intern_count$Student.institution), unique(inst_intern_count$internship))
...
links <- inst_intern_count %>%
  left_join(nodes, by = c('Student.institution' = 'name')) %>%
  rename(origin_id = id) %>%
...
# Use this to save the plot as html
# saveNetwork(file =  'forcediag.html')
```
  
The two main Institution feeders appear to be Victoria University of Wellington and Auckland University, the former resulting in half of internships.
...
Now compare the tree to the tree using the [visNetwork](https://cran.r-project.org/web/packages/visNetwork/index.html) R package visTree function. This creates an interactive tree and produces the numbers in each node.

```{r visnetwork}
# http://www.sthda.com/english/articles/33-social-network-analysis/137-interactive-network-visualization-using-r/
visNetwork::visTree(model, main = "Decision tree of events that result in successful (green) 2017 internships",
